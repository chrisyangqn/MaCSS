...
# Normalize the final risk factor
acs_data_cleaned <- acs_data_cleaned %>%
  mutate(
...
top10_risk_counties_acs <- merged_data %>%
  arrange(desc(displacement_risk_factor)) %>%  # Sort in descending order
  select(county, displacement_risk_factor) %>%  # Select relevant columns
  head(10)  # Get top 10 counties

print(top10_risk_counties_acs)

top10_risk_counties_evi <- merged_data %>%
  arrange(desc(avg_eviction_filings)) %>%  # Sort in descending order
  select(county, avg_eviction_filings) %>%  # Select relevant columns
  head(10)  # Get top 10 counties
...
ggplot(merged_data, aes(x = displacement_risk_normalized, y = avg_eviction_filings_normalized)) +
  geom_point(color = "#A833B9", alpha = 0.7, size = 3) +  # Purple points
  ...
  labs(
    ...
    caption = paste("Pearson Correlation:", round(correlation_value, 3))  # Show correlation in caption
  )

```

# Proportion of Population Under High Risk

```{r}

# Define thresholds for high-risk counties (top 25%) for ACS and eviction risk
...
# Create a combined displacement risk measure (normalized ACS + Eviction risk)
merged_data <- merged_data %>%
  mutate(
...
# Define threshold for high-risk counties using the combined measure (top 25%)
combined_risk_threshold <- quantile(merged_data$combined_risk, 0.75, na.rm = TRUE)

# Filter high-risk counties for each measure
...
# Function to calculate the proportion of each group at high risk
calculate_high_risk_proportion <- function(high_risk_data, total_data) {
  ...
  # Calculate the proportion of each group at high risk
  proportion_table <- high_risk_summary / total_summary
  proportion_table <- proportion_table %>%
    mutate(Risk_Type = NA) %>% # Placeholder to align with later binding
    select(Risk_Type, everything()) # Ensure proper column order
  
  return(proportion_table)
}

# Compute proportions for each risk measure
...
colnames(proportion_table_transposed) <- proportion_table_transposed[1, ] # First row as column names
proportion_table_transposed <- proportion_table_transposed[-1, ] # Remove first row
...
# Create the bar plot with ordered categories
ggplot(proportion_long, aes(x = Group, y = Proportion, fill = Risk_Type)) +
  geom_bar(stat = "identity", position = "dodge") + 
  scale_fill_manual(values = c("#A833B9", "#C42D92", "#FF69B4")) +  # Custom purple-pink colors
