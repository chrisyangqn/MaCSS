...
# Get ACS data for Alameda County at the census tract level
alameda_tract_income <- get_acs(
  geography = "tract",
  variables = variables,
  state = "CA",
  county = "Alameda",
...
# Summarize the median household income across census tracts in Alameda County
summary_income <- alameda_tract_income %>%
  summarise(
    ...
    sd_income = sd(estimate, na.rm = TRUE)
  )
...
summary_income_df <- as.data.frame(summary_income)

# View the summary in a table format
glimpse(summary_income_df)
summary(summary_income_df)